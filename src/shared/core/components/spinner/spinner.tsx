...
export function Spinner({ size = "small", color = "primary" }: SpinnerProps) {
  ...
  const strSpinClasses = `inline w-${sizeValues[size]} h-${sizeValues[size]} mr-2 text-gray-200 animate-spin dark:text-gray-600 ${colorClasses[color]}`;

  return (
    <div className="text-center">
      <svg
        role="status"
        className={strSpinClasses}
        viewBox="0 0 100 101"
        fill="none"
